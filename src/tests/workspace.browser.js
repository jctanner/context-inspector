// Synthetic read-only responses; never browse real workspace contents.
async (page) => {
  ...
  try {
    ...
    await context.route("**/api/workspace/file?*", route => {
    ...
    check(lists === 0, "no eager workspace scan");
    await view.locator("#nav-workspace").click();
    await view.locator("#workspace-status").filter({ hasText: "5 entries" }).waitFor();
    check(await view.locator("#session-section").isHidden(), "workspace replaces session");
    await view.locator('#workspace-files button[data-path="note.html"]').click();
    await view.locator("#workspace-file-content").filter({ hasText: "exact source" }).waitFor();
    check(await view.locator("#workspace-file-content").innerText() === source, "exact plaintext");
    check(await view.locator("#workspace-section img, #workspace-section textarea, #workspace-section input").count() === 0, "no active content/edit controls");
    check(await view.locator('#workspace-files button[data-path="link"]').isDisabled(), "links disabled");
    await view.locator("#workspace-more").click();
    await view.locator('#workspace-files button[data-path="later.txt"]').waitFor();
    check(await view.locator("#workspace-files li").count() === 6, "pagination appends");
    await view.locator('#workspace-files button[data-path="binary"]').click();
    await view.locator("#workspace-file-meta").filter({ hasText: "Binary file" }).waitFor();
    check(await view.locator("#workspace-file-content").innerText() === "", "no stale content after errors");
    await view.locator('#workspace-files button[data-path=".context"]').click();
    await view.locator('#workspace-files button[data-path=".context/config.json"]').click();
    await view.locator("#workspace-file-content").filter({ hasText: "exact source" }).waitFor();
    await view.locator("#nav-memory").click();
    check(await view.locator("#workspace-section").isHidden(), "Memory hides Workspace");
    const before = lists;
    await view.locator("#nav-workspace").click();
    check(lists === before, "navigation preserves workspace state");
    fail = true;
    await view.locator("#workspace-refresh").click();
    await view.locator("#workspace-file-meta").filter({ hasText: "no longer exists" }).waitFor();
    check(await view.locator("#workspace-file-content").innerText() === "", "refresh clears deleted file");
    await view.locator("#workspace-breadcrumbs").getByRole("button", { name: "/workspace", exact: true }).click();
    await view.locator('#workspace-files button[data-path="empty"]').click();
    await view.locator("#workspace-status").filter({ hasText: "folder is empty" }).waitFor();
    ...
    check(await view.locator("#workspace-section").isHidden(), "Session hides Workspace");
